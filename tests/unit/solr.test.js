...
describe('solr.js', () => {
  ...
  describe('querySOLR', () => {
    it('should return response object with docs', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://career.ascom.com/job/1', cif: '38652516' },
        { url: 'https://career.ascom.com/job/2', cif: '38652516' }
      ]));

      const result = await solr.querySOLR('38652516');
    ...
    it('should throw when SOLR_AUTH is missing', async () => {
      delete process.env.SOLR_AUTH;
      await expect(solr.querySOLR('38652516')).rejects.toThrow('SOLR_AUTH not set in environment');
      process.env.SOLR_AUTH = 'test:test';
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.querySOLR('38652516')).rejects.toThrow('SOLR query error: 500');
    });
  });
  ...
  describe('queryCompanySOLR', () => {
    it('should return company data', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(1, [
        { id: '38652516', company: 'ASCOM MOBILE SOLUTIONS ROMANIA S.R.L.', brand: 'ASCOM' }
      ]));

      const result = await solr.queryCompanySOLR('id:38652516');

      expect(result.numFound).toBe(1);
      expect(result.docs[0].brand).toBe('ASCOM');
    });
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.queryCompanySOLR('id:38652516')).rejects.toThrow('SOLR company query error: 401');
    });
  });
  ...
  describe('upsertJobs', () => {
    ...
    it('should throw on HTTP error', async () => {
      ...
      await expect(solr.upsertJobs([{ url: 'https://career.ascom.com/bad' }])).rejects.toThrow('SOLR upsert error: 400');
    });
  ...
  describe('deleteJobByUrl', () => {
    it('should delete a job by URL', async () => {
      ...
      await expect(solr.deleteJobByUrl('https://career.ascom.com/old-job')).resolves.not.toThrow();
    });

    it('should throw on HTTP error', async () => {
      mockFetch.mockResolvedValue(makeErrorResponse(500, 'Error'));

      await expect(solr.deleteJobByUrl('https://career.ascom.com/bad')).rejects.toThrow('SOLR delete error: 500');
    });
  });
  ...
  describe('deleteJobsByCIF', () => {
    it('should delete all jobs for a CIF', async () => {
      ...
      await expect(solr.deleteJobsByCIF('38652516')).resolves.not.toThrow();
    });

    it('should throw on HTTP error', async () => {
      mockFetch.mockResolvedValue(makeErrorResponse(500, 'Error'));

      await expect(solr.deleteJobsByCIF('38652516')).rejects.toThrow('SOLR delete error: 500');
    });
  });
  ...
  describe('Data Integrity', () => {
    it('should not have duplicate URLs for same CIF', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://career.ascom.com/job/1', title: 'Job 1', cif: '38652516' },
        { url: 'https://career.ascom.com/job/2', title: 'Job 2', cif: '38652516' }
      ]));
    ...
    it('should have valid CIF format for all jobs', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(2, [
        { url: 'https://career.ascom.com/job/1', title: 'Job 1', cif: '38652516' },
        { url: 'https://career.ascom.com/job/2', title: 'Job 2', cif: '12345678' }
      ]));
    ...
    it('should detect invalid CIF format', async () => {
      mockFetch.mockResolvedValue(makeSolrResponse(1, [
        { url: 'https://career.ascom.com/job/1', title: 'Job 1', cif: 'abc' }
      ]));
    ...
    it('should have valid status values', async () => {
      ...
      mockFetch.mockResolvedValue(makeSolrResponse(3, [
        { url: 'https://career.ascom.com/job/1', title: 'Job 1', cif: '38652516', status: 'scraped' },
        { url: 'https://career.ascom.com/job/2', title: 'Job 2', cif: '38652516', status: 'verified' },
        { url: 'https://career.ascom.com/job/3', title: 'Job 3', cif: '38652516', status: 'published' }
      ]));
